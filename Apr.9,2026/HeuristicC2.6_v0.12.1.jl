...
# Heuristic C2.6: Prioritize the item with the highest backorder level to be increase first
...
function FindProdQtyC2(Data::DefMDPData, InitInvList::Vector{Int64}, prd::Int64)
    ...
    for _ in 1:20
        ...
        if TotalSav > MaxTotalSav
            ...
            TotalInvCost = AvgInvCostAft - AvgInvCostInit
        end
    end
    ...
    if OptComparison                                                                           # If comparison with fixed costs is needed
        ...
        if TotalInvCost > FixedCost                                                              # If the total savings is less than the fixed costs,                        
            ...
            if sum(OptSol) < Data.Cap 
                ...
                if AddInvCost + TotalInvCost <= FixedCost
