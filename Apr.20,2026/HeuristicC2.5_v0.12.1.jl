...
# Heuristic C2.5: Prioritize the item with the highest backorder level to be increase first
...
# Define the function 
function FindProdQtyC2(Data::DefMDPData, InitInvList::Vector{Int64}, prd::Int64)
    ...
    for _ in 1:20
        ...
        if (prd >= 2) && all(CurrInvList .> ExpFutDemand)
            return zeros(Int64, Data.Nᵢ)
        elseif (prd == 2) && all(CurrInvList .> 0)
            return zeros(Int64, Data.Nᵢ)
        end
        ...
        for i in 1:Data.Nᵢ
            ...
            InvCost = abs(AvgInvCostList2[i] - AvgInvCostList1[i])
            FixedCost = Data.BH[i]*Data.pd + Data.c
            #println("Savings: $Savings, Bk1: $(AvgBkCostList1[i]), Bk2: $(AvgBkCostList2[i]), Inv1: $(AvgInvCostList1[i]), Inv2: $(AvgInvCostList2[i]) ")
            if (Savings[i] < 0) || InvCost >= FixedCost
                ToBeImprove[i] = false
            end
        end
        ...
        while true
            ...
            InvCost = abs(AvgInvCostList2[MaxSvIdx] - AvgInvCostList1[MaxSvIdx])
            FixedCost = Data.BH[MaxSvIdx]*Data.pd + Data.c
            ...
            if (Savings[MaxSvIdx] < 0) || InvCost >= FixedCost # If the increase inventory cost exists the 
    ...
    if OptComparison                                                                           # If comparison with fixed costs is needed
        ...
        if MaxTotalSav < FixedCost                                                              # If the total savings is less than the fixed costs,                        
            ...
            if sum(OptSol) < Data.Cap 
                AddInvCost = 0.0
                AddProd = zeros(Int, Data.Nᵢ) 
                CurrInvList = max.(InitInvList .- ExpFutDemand, Data.MinI) .+ OptSol
                for i in 1:Data.Nᵢ
                    InvPrd = min((CurrInvList[i]/ExpFutDemand[i]), prd-2)
                    EndInv = CurrInvList[i] - ExpFutDemand[i]*InvPrd
                    EndBk = 0
                    ...
                    #println("InvPrd: $InvPrd, End Inv: $EndInv")
                    if EndInv > 0
                        AvgInvCost1 = ((((CurrInvList[i] + EndInv) * InvPrd)/2)* Data.h[i]) + EndInv*Data.h[i] 
                        EndBk = 0
                        #println("Avg Inv Cost: $AvgInvCost, Avg Bk Cost: $AvgBkCost")
                    else
                        AvgInvCost1 = ((((CurrInvList[i] + EndInv) * InvPrd)/2)* Data.h[i])
                        BkPrd = prd - 2 - InvPrd
                        EndBk = ExpFutDemand[i]*BkPrd
                    end
                    if EndBk > 0
                        AddProd[i] = ceil(EndBk)
                        CurrInvList[i] += ceil(EndBk)
                        InvPrd = min((CurrInvList[i]/ExpFutDemand[i]), prd-2)
                        EndInv = CurrInvList[i] - ExpFutDemand[i]*InvPrd
                        if EndInv > 0
                            AvgInvCost2 = ((((CurrInvList[i] + EndInv) * InvPrd)/2)* Data.h[i]) + EndInv*Data.h[i] 

                            #println("Avg Inv Cost: $AvgInvCost, Avg Bk Cost: $AvgBkCost")
                        else
                            AvgInvCost2 = ((((CurrInvList[i] + EndInv) * InvPrd)/2)* Data.h[i])
                        end
                        AddInvCost += (AvgInvCost2 - AvgInvCost1)*Data.h[i]
                    end
                end
                Production = (any(AddProd .> 0) ? 1 : 0)                                                # Fixed production cost is incurred if there is any production
                Powder = maximum((AddProd .> 0) .* Data.BH)                                             # Powder cost is based on the maximum build height among all items being produced
                ProdCost = Production * Data.c
                PowderCost = Powder * Data.pd
                FixedCost = ProdCost + PowderCost
                
                if (AddInvCost <= FixedCost) && (sum(OptSol .+ AddProd) <= Data.Cap)
                    return OptSol .+ AddProd
                else
                    return OptSol
                end
            else
...
#init_inv = [2, 2, 2, 2]
#prd = 5
#sol = FindProdQtyC2(Data, init_inv, prd)
